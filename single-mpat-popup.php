...
Tags: 			hbbtv
*/
		$buttonFuncs = get_post_meta(get_post()->ID,'_mpat_popupContent',true);
		...
		function hbbTVButton($btn_name, $func_id){
			...
			<div  class="function_button">
				<img src="<?php echo (get_bloginfo('template_url').'/shared/assets/button'. $btn_name.'.png') ?>"></img>
				<div class="label"><?php echo $title; ?></div>
				<script>
					if (!userFunctions['func'+<?php echo $func_id;?>]){
						userFunctions['func'+<?php echo $func_id;?>] = function(){
							<?php echo get_post_meta($func_id,'_mpat_functionContent',true)?>;
						}
					}
				jQuery('#popup').nav(<?php echo $btn_name?>,userFunctions['func'+<?php echo $func_id;?>]);
				</script>
			</div>
			<?php
		}
