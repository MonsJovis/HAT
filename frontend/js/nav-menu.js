...
jQuery(document).ready(function($){
	...
	$('.menu-item-object-mpat_popup').navEnter(function() {
		console.log('navEnter');
	...
	$('.menu-item-object-mpat_function').navEnter(function(){
		console.log('navEnter');
	...
	$generalNav.nav("Red", function(){
		if (appActive){
			...
			if($("[data-type=broadcast]").length){
				$("[data-type=broadcast]").empty().html("<object id='broadcast' type='video/broadcast' height='100%' width='100%'></object><script type='text/javascript'>window.setTimeout( function() { document.getElementById('broadcast').bindToCurrentChannel(); }, 10); </script>");
			}
			hbbtvlib_show();
			appActive = true;
		}
