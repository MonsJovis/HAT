...
function mpat_button_control_menu() {

	add_menu_page( 'MPAT Button Control', 'Button Control', 'manage_options', 'mpat_button_control', 'mpat_button_control_callback', 'dashicons-list-view' );
}

function mpat_button_control_callback() {
	...
	function action_editor($menu_items){
		...
		foreach ( (array) $menu_items as $key => $menu_item ) {
			if (get_post_meta($menu_item->ID,'_menu_item_is_hide',true)!==''){
				...
				$button = get_post_meta($menu_item->ID,'_menu_item_mpat_button',true);
		...
		<div class= "btn_control_pages_buttons" style="margin: 20px 0px;">
			<div class=class="btn_control_pages_buttons_pages" style="padding: 15px; height: 25px; text-align: center;">
				<div class="mpat-menu-button" style="width:10%;">
				...
				<ul id="menu-item-list" class="droped-list">
					...
					if ($hide_button_id===false){
						...
						<li id="hide-button" class="drag-menu-item">
							<div class="mpat-menu-button">
								<div  ondragstart='event.preventDefault()'><?php mpat_button('Red');?></div>
							</div>
							<div class="mpat-menu-post" draggable="true">
					...
					foreach ( (array) $menu_items as $key => $menu_item ) {
						...
						$usedButton = get_post_meta($menu_item->ID,'_menu_item_mpat_button',true);
						if ($usedButton==''){
						...
						<li class="drag-menu-item" >
							<div class="mpat-menu-button">
								<div class="drag-button"><?php mpat_button($usedButton);?></div>
							</div>
							<div class="mpat-menu-post" post-type="<?php echo $post_type?>" post-id="<?php echo  $menu_item->object_id; ?>" draggable="true">
					...
					<li class="drag-menu-item placeholder" style="">
						<div class="mpat-menu-post placeholder" draggable="true">
	...
	function mpat_button($name){
		?>
		<div class="mpat-button" button-name="<?php echo $name?>">
			<img src="<?php echo get_template_directory_uri().'/shared/assets/button'.$name.'.png'; ?>"></img>
		</div>
		<?php
	}
			...
			<ul class="btn_control_pages_list draggable-list" post-type="<?php echo $post_type?>"><?php 
				...
				foreach ( $posts as $post ) {
					...
					<li class="drag-menu-item unused">
						<div class="mpat-menu-post" post-type="<?php echo $post->post_type;?>" post-id="<?php echo  $post->ID; ?>"  draggable="true">
	...
	<input type="submit" name="submit" id="submit" class="button button-primary" value="Save Changes"  onclick="saveButtonControlData('<?php echo get_template_directory_uri(); ?>/mpat-button-control-save.php')"/>
