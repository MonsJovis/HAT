...
Mpat_Utilities::get_template_parts( array( 'frontend/parts/html-header', 'frontend/parts//header' ) );
	...
	<script type="text/javascript">
		jQuery(document).ready(function($){
			$("#hbbtv_app").css("background-image", "url(<?php echo get_template_directory_uri(); ?>/frontend/assets/bgr_sandmann.png)");
			...
			loadSandmannContent = function() {
				...
				$.ajax({
						type: "GET",
						url: '/wordpress/wp-content/themes/MPAT/getContent.php?url=www.sandmann.de/static/san/app_tv/filme.xml',
						dataType: "xml"
					})
					.done(function(data){
			...
			loadSandmannSource = function(url, callback) {
				...
				$.ajax({
						type: "GET",
						url: "/wordpress/wp-content/themes/MPAT/getContent.php?url="+url,
						dataType: 'xml'
					})
					.done(function(data){
						result = data;
